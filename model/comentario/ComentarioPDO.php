...
class Daocomentario extends DB
{
    ...
    public function listar()
    {
        ...
        foreach ($this->filas as $fila) {
            ...
            $come->__set("titulo", $fila['titulo']); //Le asignamos a las propiedades del objetos los campos de esa fila
    ...
    /**
     * Obtener comentarios por ISBN
     *
     * @param string $isbn13
     */

    public function obtenerPorISBN($isbn13)
    {
        $consulta = "SELECT * FROM comentario WHERE producto_isbn_13= :isbn13 ORDER BY fecha DESC";
        $param = array();
        $param[":isbn13"] = $isbn13;
        $this->consultaDatos($consulta, $param);
        foreach ($this->filas as $fila) {
    ...
    { //Se introduce un objeto por parametro que se insertará
        $consulta = "INSERT INTO comentario VALUES (:id, :usuario_id, :producto_isbn_13, :titulo, :comentario, :fecha ) ";
        $param = array();
        ...
        $param[":titulo"] = $come->__get("titulo"); //Le asignamos a las propiedades del objetos los campos de esa fila
    ...
    { //Se introduce un objeto por parametro que se insertará
        $consulta = " UPDATE comentario SET usuario_id= :usuario_id, producto_isbn_13= :producto_isbn_13, titulo= :titulo, comentario= :comentario, fecha= :fecha WHERE id= :id";
        $param = array();
        $param[":id"] = $come->__get("id"); //Le asignamos a las propiedades del objetos los campos de esa fila
        $param[":usuario_id"] = $come->__get("usuario_id"); //Le asignamos a las propiedades del objetos los campos de esa fila
        $param[":producto_isbn_13"] = $come->__get("producto_isbn_13"); //Le asignamos a las propiedades del objetos los campos de esa fila
